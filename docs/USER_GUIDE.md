...
**Forma degli input:** i parametri-array (`elementIds`) sono array JSON di interi; gli `inputPoints` (in `create_steel_connection`) sono oggetti `{"x":..,"y":..,"z":..}` in mm, **non** array piatti.

**Anteprima (`dryRun`):** solo *alcuni* strumenti di scrittura supportano `dryRun: true` per un'anteprima senza modifiche; **tutti** gli strumenti di scrittura mostrano comunque una conferma nativa di Revit (TaskDialog) prima di scrivere. Gli strumenti che supportano `dryRun` sono: `create_generic_steel_connection`, `create_steel_connection`, `set_steel_connection_type`, `delete_steel_connection`, `create_steel_structural_connection_type`, `create_steel_connection_handler_type`, `create_default_steel_connection_handler_type`, `set_steel_connection_type_family_symbol`, `manage_steel_approval_type` (action `create`), `add_steel_fabrication_info`, `add_steel_solid_cut`, `add_steel_instance_void_cut`. I mutatori `modify_steel_connection_inputs`, `set_steel_connection_approval`, `set_steel_connection_status`, `set_steel_connection_default_order`, `remove_steel_solid_cut`, `set_steel_solid_cut_face_splitting`, `remove_steel_instance_void_cut`, `set_steel_fabrication_unique_id` **non** offrono un'anteprima: confermano e poi scrivono.
...
- **`set_steel_connection_type`** non è un setter in-place (l'API non lo espone): **ricrea** la connessione (elimina + ricrea con il nuovo tipo). Preserva gli elementi connessi e fa un **restore best-effort** dello stato scrivibile (`approvalTypeId`, `codeCheckingStatus`, `overrideTypeParams`, `singleElementEndIndex`), riportato in `restoredFields`/`lostFields`. I **punti/riferimenti di input non sono ricreati** (l'API non espone un costruttore pubblico per `ConnectionInputPoint`/`Reference`): con `dryRun: true` la risposta elenca `willPreserve`/`willLose` e uno `stateSnapshot`.
...
- **`set_steel_fabrication_unique_id`** è **sperimentale**: scrive `SteelElementProperties.UniqueID` tramite un *setter non pubblico* invocato per reflection (la risposta riporta `experimental: true`). Può smettere di funzionare su versioni future di Revit; in tal caso restituisce un Fail strutturato anziché crashare.
