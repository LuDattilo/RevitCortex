...
**Visual Power BI Desktop (Phase 2C):** Il file `powerbi-visual/dist/revitcortexselectionvisual1A2B3C4D.1.0.0.2.pbiviz` è un custom visual importabile in Power BI Desktop che aggiunge un pulsante grande **"Seleziona in Revit (N)"** (usa automaticamente le righe evidenziate da un altro visual se presenti, altrimenti tutto il filtro corrente), un pulsante secondario **"Isola in Revit"**, e un indicatore di connessione al listener HTTP su porta 27016. Non richiede autenticazione MSAL — comunica direttamente con Revit tramite richiesta POST locale. La colonna ElementId va trascinata nel campo "Element ID"; il data role è `kind: Grouping` quindi PBI non aggrega mai (niente "Count of...").
