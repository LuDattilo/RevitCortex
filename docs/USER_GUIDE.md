...
| `create_grid` | Crea o gestisce griglie (singole o matrice con passo X/Y). Tutte le azioni (create/rename/delete) sono preview-first (anteprima con dryRun=true di default; dryRun=false per applicare, richiede conferma). | "Crea una griglia 5x4 con passo 6000 mm, prima in anteprima" |
| `create_level` | Crea o gestisce livelli con quota e flag di piano edificio. Tutte le azioni (create/set/rename/delete) sono preview-first (anteprima con dryRun=true di default; dryRun=false per applicare, richiede conferma). | "Aggiungi un livello a quota 3500 mm chiamato Piano 2, prima in anteprima" |
