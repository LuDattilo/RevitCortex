...
- **Phase 4 — DONE** (the formerly-deferred list, now closed): 4a floor/filled_region holes (`31ebed7`); 4b arc walls/beams + compound-structure wrapping (`41aeb6d`); 4c purge_unused → view templates + filters (`d1c9ced`); 4d copy_elements cross-document + create_view_filter multi-rule (`e45eec8`); 4e set_material_properties asset assignment + real ArrayElement/BeamSystem (`ab0bb0b`).
- **VERIFIED-NOT-POSSIBLE (documented, not faked):** create_dimensions radial/diameter/angular — the Revit API exposes NewRadialDimension/NewDiameterDimension/NewAngularDimension only on FamilyItemFactory (family editor), not in a project document. manage_worksets open/close — no live-document API. These are genuine API limits, stated in the tool descriptions.
- **STILL deferred (genuinely lower value, not blocking "all commands"):** create_surface_based_element inner loops + per-edge slope; create_line_based_element location-line/justification; set_compound_structure deck profile/variable-thickness; manage_additional_settings line/fill pattern *create*; manage_project_units decimal-symbol/rounding detail. Small, niche; left for a future pass.
