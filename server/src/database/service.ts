...
export function storeProject(data: ProjectData): number {
  ...
  if (existing) {
    dbRun(`
      UPDATE projects SET
        project_path = ?, project_number = ?, project_address = ?,
        client_name = ?, project_status = ?, author = ?,
        last_updated = ?, metadata = ?
      WHERE id = ?
    `, [
      data.project_path || null, data.project_number || null, data.project_address || null,
      data.client_name || null, data.project_status || null, data.author || null,
      timestamp, metadata, existing.id,
    ]);
    return existing.id;
  }
  ...
  `, [
    data.project_name, data.project_path || null, data.project_number || null,
    data.project_address || null, data.client_name || null, data.project_status || null,
    data.author || null, timestamp, timestamp, metadata,
  ]);
...
function storeRoom(projectId: number, data: RoomData): number {
  ...
  if (existing) {
    dbRun(`
      UPDATE rooms SET
        room_name = ?, room_number = ?, department = ?, level = ?,
        area = ?, perimeter = ?, occupancy = ?, comments = ?,
        timestamp = ?, metadata = ?
      WHERE id = ?
    `, [
      data.room_name || null, data.room_number || null, data.department || null,
      data.level || null, data.area || null, data.perimeter || null,
      data.occupancy || null, data.comments || null, timestamp, metadata, existing.id,
    ]);
    return existing.id;
  }
  ...
  `, [
    projectId, data.room_id, data.room_name || null, data.room_number || null,
    data.department || null, data.level || null, data.area || null,
    data.perimeter || null, data.occupancy || null, data.comments || null,
    timestamp, metadata,
  ]);
  return dbLastInsertRowid();
}
...
export function storeRoomsBatch(projectId: number, rooms: RoomData[]): number {
  for (const room of rooms) storeRoom(projectId, room);
  return rooms.length;
}
...
export function getAllRoomsWithProject() {
  return dbAll(`
    SELECT r.id, r.project_id, r.room_id, r.room_name, r.room_number,
      ...
      r.comments, r.timestamp, r.metadata, p.project_name, p.project_number
...
export function getStats() {
  ...
  return { total_projects: projectCount?.count ?? 0, total_rooms: roomCount?.count ?? 0 };
}
